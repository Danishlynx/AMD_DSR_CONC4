...
**Harness**: `kimbochen/dsr1_benchmark.cpp` — ISL=8192, OSL=1024, num_prompts=40, max-concurrency=4, num_warmups=8, 4-iter median(2,3,4); GSM8K via lm_eval N=3 median
...
> **Caveat**: Phase 11 v3 was added 3 days *after* this measurement. The lever is concurrency-agnostic (sampler change), so it would likely improve CONC=32 too — but that improvement is **projected, not measured**.
>
> **Why this matters for follow-up**: failing gates at CONC=32 miss by only **5.8%** (E2E) and **1.8%** (Tput) — vs much larger gaps at CONC=4. **CONC=32 4/4 is likely reachable before CONC=4 4/4** as additional levers stack.
...
The other levers in the trajectory are either pre-existing flags AMD/ATOM already ship (which were just toggled ON), or CLI configurations — those are documented for reproducibility but are not source modifications in this PR.

**Files modified by this PR** (in ATOM_main):
- `atom/utils/envs.py` — adds `ATOM_ENABLE_PER_PHASE_RELAXED_MTP` env flag (+2 lines)
- `atom/model_engine/model_runner.py` — `spec_phase` int8 tensor allocation + setter registration + prefill-slot reset (+18 lines)
...
| CONC=32 official harness data (Apr 27) | `docs/Daily Updates/MASTER.md` §"A27 CONC=32 reference" |
...
- [`README.md`](README.md) — top-level overview + repo structure
- [`PR_DESCRIPTION.md`](PR_DESCRIPTION.md) — full PR description (item 2)
- [`TECHNICAL_APPROACH.md`](TECHNICAL_APPROACH.md) — profiling methodology + decision tree (item 5)
- [`docs/Daily Updates/REPRODUCE.md`](docs/Daily%20Updates/REPRODUCE.md) — single-command reproduction recipe
- [`docs/Daily Updates/OFFICIAL_HARNESS.md`](docs/Daily%20Updates/OFFICIAL_HARNESS.md) — kimbochen harness contract
