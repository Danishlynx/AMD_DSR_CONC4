...
class EagleProposer:
    ...
    def __init__(
        ...
        # Phase 4A v4: Drafter HIP graph capture — static buffers (addresses baked into graph)
        _dg_max_bs = max(self.cudagraph_batch_sizes) if self.cudagraph_batch_sizes else max_bs
        self._dg_max_bs = _dg_max_bs
        self._drafter_graphs = {}              # bs -> CUDAGraph or None (None = capture failed)
        self._dg_input_ids = torch.zeros(_dg_max_bs, dtype=torch.int64, device=self.device)
        self._dg_positions = torch.zeros(_dg_max_bs, dtype=torch.int64, device=self.device)
        _hidden_size = self.speculative_config.draft_model_hf_config.hidden_size
        self._dg_hidden = torch.zeros(_dg_max_bs, _hidden_size, dtype=self.dtype, device=self.device)
        self._dg_hidden_out = {}               # bs -> model output tensor (graph output slot)
        self._dg_logits = {}                   # bs -> logits tensor (graph output slot)
        self._dg_stream = torch.cuda.Stream()  # dedicated capture stream
    ...
    def _capture_draft_graph(self, bs):
        """Capture drafter forward+compute_logits into a HIP graph for iter 1,2 replay.

        Must be called AFTER eager i=0 has transitioned metadata into decode shape
        (qseqlen=1). self.runner.forward_vars tensors are address-stable across
        decode steps, so the graph only needs these static input buffers registered.

        The capture is wrapped in aiter's graph_capture() context so NCCL AllReduce
        IPC peer-pointer slots get bound to the graph instance — missing this was
        the v2/v3 NULL-pointer crash root cause.
        """
        from aiter.dist.parallel_state import graph_capture  # LAZY: avoid top-level import cascade

        if bs in self._drafter_graphs:
            return

        try:
            with graph_capture() as _gc:
                stream = self._dg_stream
                stream.wait_stream(torch.cuda.current_stream())
                with torch.cuda.stream(stream):
                    # 3 warmup passes so JIT / flydsl scratch buffers settle before capture
                    for _ in range(3):
                        _w_out = self.model(
                            input_ids=self._dg_input_ids[:bs],
                            positions=self._dg_positions[:bs],
                            hidden_states=self._dg_hidden[:bs],
                        )
                        _ = self.model.compute_logits(_w_out)
            ...
            self._drafter_graphs[bs] = graph
            self._dg_hidden_out[bs] = out
            self._dg_logits[bs] = logits
            logger.info(f"[DG v4] Captured drafter graph bs={bs}")
        except Exception as e:
            logger.warning(f"[DG v4] FAIL bs={bs}: {type(e).__name__}: {e}")
            self._drafter_graphs[bs] = None  # stay eager permanently for this bs
    ...
    def propose(
        ...
        # Phase 4A v4: is drafter HIP graph available for this call?
        allow_graph = (
            self.use_cuda_graph
            and bs in self.cudagraph_batch_sizes
        ...
        for i in range(self.mtp_k):
            with record_function(f"draft[{i}/{self.mtp_k} bs={bs}]"):
                # Phase 4A v4: iter 1,2 use graph replay if available; i=0 always eager
                _graph = self._drafter_graphs.get(bs) if (i > 0 and allow_graph) else None
                if _graph is not None:
                    # Copy runtime inputs into static buffers, replay graph
                    self._dg_input_ids[:bs].copy_(input_ids)
                    self._dg_positions[:bs].copy_(positions)
                    self._dg_hidden[:bs].copy_(hidden_states)
                    _graph.replay()
                    sample_hidden_states = self._dg_hidden_out[bs]
                    logits = self._dg_logits[bs]
                else:
                ...
                if i < self.mtp_k - 1:
                    ...
                    if i == 0:  # Skip full metadata rebuild at i=1 (saves ~0.5-1ms)
                    ...
                    # Phase 4A v4: capture drafter graph ONCE, right after i=0 puts
                    # metadata into decode shape. Current propose() call continues
                    # eager; subsequent calls with same bs use graph replay at i>0.
                    if i == 0 and allow_graph and bs not in self._drafter_graphs:
                        self._capture_draft_graph(bs)
