...
struct HkMlaDecodeFwdTraitsH32
{
    ...
    static constexpr int32_t kNumWarps      = 2;  // h32: 2 warps for kBlockM=32 with kTileM=16
    static constexpr int32_t kNumThreads    = kNumWarps * ckt::get_warp_size();
    // h32: simulate 8-warp spatial partitioning with 2 real warps via virtual-warp loop.
    // Buffer managers (KvManagerV2, VtManagerV1) encode a 2x4 warp grid for LDS access;
    // at kNumWarps=2, we loop 4 times with virtual_warp_idx = warp_idx + vi*kNumWarps
    // so 2 real warps cover all 8 original warp positions: warp 0 does {0,2,4,6},
    // warp 1 does {1,3,5,7}.
    static constexpr int32_t kVirtualWarps   = 8;
    static constexpr int32_t kVirtualPerReal = kVirtualWarps / kNumWarps;  // 4 at kNumWarps=2
    static constexpr int32_t kOccupancy     = 1;  // h32: keep occupancy=1 to preserve VGPR budget (slots up to 255 used by oaccu)
    static constexpr int32_t kBlockM        = 32; // h32: kQoNumHead=32, kBlockM matches
    ...
    static_assert(kBlockM == kQoNumHead, "h32 traits: kBlockM must match kQoNumHead (32)");
    static_assert(kNumWarps == 2, "h32 traits: kNumWarps=2 for 2-warp-native port");
    static_assert(kTileM == 16, "h32 traits: kTileM=16 for MFMA fp8 16x16x32");
    static_assert(kVirtualPerReal == 4, "h32 traits: virtual/real ratio must be 4 (8 warps simulated by 2)");
...
// Note: HkMlaDecodeFwdParams, PvGemmEpilogueType, pack_4f32_to_fp8, max_8
// are defined in mi3xx_v32_fwd_decode_h128_fp8_fp8.cuh (included first in hk_decode_fwd.cu).
// Reusing those definitions here avoids duplicate-symbol link errors.

template <typename T>
__global__ __launch_bounds__(T::kNumThreads, T::kOccupancy) __attribute__((
    amdgpu_num_vgpr(72))) void kn_mla_v32_fwd_decode_h32_fp8_fp8(HkMlaDecodeFwdParams<T> params)
{
    using q_t     = T::q_t;
    using kv_t    = T::kv_t;
    using out_t   = T::out_t;
    ...
    {
        // v6 fix: explicit sync between work_idx iterations.
        // At qseqlen>1, multiple work units per kernel launch. Without this barrier,
        // virtual-warp loops from previous work_idx may not fully drain before next
        // iter's outer K load → stale LDS data → garbage output for some positions.
        ...
        // H32: Q buffer is gl_q<q_t, -1, kNumTilesM=2, kTileM=16, 576>.
        // At kNumWarps=2 kNumTilesM=2, each real warp loads its own tile (warp_idx directly
        // indexes the 2-slot kNumTilesM dimension). No virtual-warp loop — would overflow.
        q_manager.template load_q_to_gpr<k_q_nope_begin, k_q_rope_begin>(
            params.query, warp_idx, qo_start, p_lds_q);
        __builtin_amdgcn_sched_barrier(0);

        // H32 v3: virtual-warp loop for K async_load, matching V store layout.
        // At kNumWarps=2 with kVirtualPerReal=4, 2 real warps write to 8 virtual-warp LDS slots.
        // Requires LDS sized for 8 warps (see kSzLdsKv override below).
        #pragma unroll
        ...
        auto mla_main = [&]<bool kIsFirstIter,
            ...
            // H32 v4: full-tile next-K prefetch via virtual-warp loop. Replaces the
            // chunked async_load_k_tile prefetch which only fills 2 of 8 LDS slots.
            // Trades chunked-prefetch overlap for correctness — next iter's V load
            // now reads coherent K LDS data at all 8 virtual warp positions.
            // The chunked async_load_k_tile sites below are no-ops (vi=0 only no-op).
            if constexpr(kIsLastIter == false) {
                #pragma unroll
                for (uint32_t vi = 0; vi < T::kVirtualPerReal; vi++) {
                    const uint32_t vwarp_k = warp_idx + vi * T::kNumWarps;
            ...
            // v4: chunked prefetch (was line 288) NEUTRALIZED — full prefetch above covers it.
            ...
            ckt::static_for<0, num_nope_iter, 1>{}([&](auto idx) {
                ...
                // v4: chunked prefetch NEUTRALIZED — full-tile virtual-warp prefetch
                // happens at top of mla_main. Per-iter chunk would overflow LDS at virtual
                // warp positions if executed.
            ...
            // H32 virtual-warp loop for V load/transpose/store.
            // Each real warp loads + transposes + stores 4 virtual-warp slices sequentially.
            // After this loop, all 8 virtual-warp positions of LDS-transposed V are populated.
            // Subsequent PV-GEMM via load_transposed_v_to_gpr (which uses compile-time
            // col offsets, not warp_idx) reads correctly.
            // NOTE: This serializes V load after softmax. Loses h128's parallel load-softmax
            // overlap. Acceptable perf cost for correctness; future optimization can re-add overlap.
            v8ui v;
            ...
            // H32: virtual-warp store loop. Each real warp stores its own transposed v
            // to the LDS slot for virtual warp 0, then loads/transposes/stores 3 more
            // virtual warp slots via additional load_v_to_gpr calls.
            vt_manager.store_transposed_v_to_lds(p_lds_vt, warp_idx, v);
            // Additional virtual-warp passes for remaining 3 slices
            #pragma unroll
            ...
            ckt::static_for<0, num_pv_iter, 1>{}([&](auto idx) {
                ...
                if constexpr(kIsLastIter)
                {
                    ...
                    {
                        // H32: output uses warp_idx for row offset (kTileM rows per warp).
                        // At kBlockM=32 kTileM=16 kNumWarps=2, 2 real warps cover 32 output rows natively.
                        // NO virtual-warp loop needed here — each real warp writes its tile rows.
...
void mla_v32_fwd_decode_h32_fp8_fp8(torch::Tensor& query,
                                     ...
                                     torch::Tensor& final_output)
{
    ...
    kn_mla_v32_fwd_decode_h32_fp8_fp8<Traits>
        <<<grid, Traits::kNumThreads, lds_size, stream>>>(params);
}

void hk_mi3xx_mla_v32_fwd_decode_h32_fp8_fp8(torch::Tensor& query,
                                              torch::Tensor& kv_buffer,
                                              const torch::Tensor& qo_indptr,
                                              const torch::Tensor& kv_indptr,
                                              const torch::Tensor& kv_page_indices,
                                              ...
                                              torch::Tensor& final_output)
{
    ...
    {
        using Traits = HkMlaDecodeFwdTraitsH32<hk::fp8e4m3, hk::fp8e4m3, hk::bf16, 32>;
        mla_v32_fwd_decode_h32_fp8_fp8<Traits>(query,
                                                kv_buffer,
                                                qo_indptr,
                                                kv_indptr,
                                                kv_page_indices,
    ...
    {
        TORCH_CHECK(false,
                    "hk_mi3xx_mla_v32_fwd_decode_h32_fp8_fp8 doesn't support q type ",
