...
//
// V9: HK qh32 with mfma_scale_f32_16x16x128_f8f6f4 upgrade in NoPE GEMM.
//
// - NoPE QK^T: 8 iters × 2 mfma_1616x32 → 2 iters × 2 mfma_1616x128
//   (4× fewer MFMA calls with 4× K-depth each; 2× latency per call at CDNA4)
//   Net ~2× NoPE GEMM throughput.
// - RoPE: unchanged (K=64 too narrow for 128-wide payoff).
// - PV: unchanged (K-reduction axis = kBlockN = kv_len chunk = 32; cannot widen
//   without 4× LDS budget and kv_len restructuring — out of scope).
//
// Requires: load_k_wide_to_gpr in KvManagerV2 and lds_2_gpr_wide in QManagerV4.
...
struct HkMlaDecodeFwdTraitsH32V9
{
    ...
    static constexpr int32_t kBlockN        = 32;   // K-reduction width for narrow MFMA (RoPE + PV)
    static constexpr int32_t kBlockK        = 32;   // unchanged for RoPE + PV narrow paths
    static constexpr int32_t kBlockN_Wide   = 128;  // V9: NoPE MFMA K-reduction width
    static constexpr int32_t kTileM         = kBlockM / kNumWarps; // 16
    static constexpr int32_t kNumTilesM     = kBlockM / kTileM;    // 2
    static constexpr int32_t kRoundMode     = 1;

    static_assert(kBlockM == kQoNumHead);
    static_assert(kNumWarps == 2);
    static_assert(kTileM == 16);
    static_assert(kVirtualPerReal == 4);

    using q_t   = q_t_;
    using kv_t  = kv_t_;
    using out_t = out_t_;
    using gl_q = hk::gl<q_t, -1, kNumTilesM, kTileM, kQkHeadDim>;
    using gl_kv = hk::gl<kv_t, -1, kPageSize, kKvNumHead, kQkHeadDim>;
...
__global__ __launch_bounds__(T::kNumThreads, T::kOccupancy) __attribute__((
    amdgpu_num_vgpr(72))) void kn_mla_v32_fwd_decode_h32_fp8_fp8_v9(HkMlaDecodeFwdParams<T> params)
{
    using q_t     = T::q_t;
    using kv_t    = T::kv_t;
    using out_t   = T::out_t;
    ...
    // ------------------------------------------------------------------
    // Register map (total 256 VGPRs at kOccupancy=1):
    // v7 layout preserved for PV/RoPE/oaccu/p_*; NoPE wide tiles live in
    // previously-unused low register range.
    //
    //   255..128  = oaccu                  (k_o_sz = 128)
    //   127..120  = p_comp                 (8)
    //   119..118  = p_mfma                 (2)
    //   117..114  = kv_1 narrow (PV/RoPE)  (4)
    //   113..110  = kv_0 narrow (PV/RoPE)  (4)
    //   109..106  = q_rope                 (4)
    //   105.. 74  = q_nope (shared by NoPE narrow refs)  (32)
    //    73.. 58  = kv_1_wide (NoPE, rt_16x128_s)  (16)
    //    57.. 42  = kv_0_wide (NoPE, rt_16x128_s)  (16)
    //    41.. 26  = q_1_wide (NoPE scratch)   (16)   -- unused; see comment below
    //    25.. 10  = q_0_wide (NoPE scratch)   (16)   -- unused
    //
    // NOTE: We REUSE q_nope (105..74, 32 VGPRs) for wide q tiles by treating
    // it as 2 iters × 16 VGPRs each (q_0 8 + q_1 8). The load_q_to_gpr
    // produces narrow-tile layout; for wide consumption the NoPE loop
    // reloads via lds_2_gpr_wide into the q_nope VGPR range.
    // ------------------------------------------------------------------

    constexpr uint32_t k_o_sz      = 128;
    constexpr uint32_t k_p_mfma_sz = 2;
    constexpr uint32_t k_p_comp_sz = 8;
    constexpr uint32_t k_kv_size   = 4;          // PV/RoPE narrow
    constexpr uint32_t k_q_rope_sz = 4;
    constexpr uint32_t k_q_nope_sz = 32;
    constexpr uint32_t k_kv_wide_size = 16;       // NoPE wide (8 VGPRs × 2 rows-halves)
    ...
    // V9 wide-tile register ranges (low registers, previously unused).
    constexpr uint32_t k_kv_0_wide_begin = 0;
    constexpr uint32_t k_kv_0_wide_end   = k_kv_0_wide_begin + k_kv_wide_size - 1;
    constexpr uint32_t k_kv_1_wide_begin = k_kv_0_wide_end + 1;
    constexpr uint32_t k_kv_1_wide_end   = k_kv_1_wide_begin + k_kv_wide_size - 1;
    ...
    // V9: Wide NoPE kv ranges. Each wide tile = rt_16x128_s (8 VGPRs/lane per base tile).
    // Outer shape = kBlockK=32 rows × kBlockN_Wide=128 cols = 2 base tiles (row-wise) × 8 VGPRs
    // = 16 VGPRs. split_many_t chunk size = 8 (registers_per_thread for rt_16x128_s base).
    using kv_0_wide_ranges = hkdart::split_many_t<
        hkdart::type_list<hkdart::range<k_kv_0_wide_begin, k_kv_0_wide_end>>, 8>;
    using kv_1_wide_ranges = hkdart::split_many_t<
        hkdart::type_list<hkdart::range<k_kv_1_wide_begin, k_kv_1_wide_end>>, 8>;
    ...
    hkdart::clobber<kv_0_wide_ranges>();
    hkdart::clobber<kv_1_wide_ranges>();

    // V9 experiment: revert to QManagerV3 (VRAM-direct narrow Q load).
    // Hypothesis: mfma_1616128 accepts the "narrow-interleaved" lane layout produced
    // by 4 adjacent rt_16x32_s tiles (lane owns cols {0-7, 32-39, 64-71, 96-103} of
    // its row) as a valid rt_16x128_s input. If true, no wide Q load is needed — we
    // just reinterpret the existing 32-VGPR q_nope data as 4 rt_16x128_s tiles.
    // If false, MFMA will produce wrong math (diagnosable via smoke test mismatch).
    ...
    // Narrow tiles (PV + RoPE, unchanged from v7)
    ...
    // V9: Wide NoPE tiles. rt_16x128_s base, outer T::kBlockK=32 rows, T::kBlockN_Wide=128 K-cols.
    hk::art<kv_t, T::kBlockK, T::kBlockN_Wide, hk::row_l, hk::rt_16x128_s, kv_0_wide_ranges> kv_0_wide;
    hk::art<kv_t, T::kBlockK, T::kBlockN_Wide, hk::row_l, hk::rt_16x128_s, kv_1_wide_ranges> kv_1_wide;
    ...
    for(int32_t work_idx = work_start_idx; work_idx < work_end_idx; ++work_idx)
    {
        ...
        auto mla_main = [&]<bool kIsFirstIter,
            ...
            // -----------------------------------------------------------------
            // V9: NoPE QK^T via mfma_scale_f32_16x16x128_f8f6f4 (kBlockN_Wide=128).
            // 2 iterations × 2 mma_ABt calls × 1 mfma_1616128 per call = 4 MFMA.
            //   (vs v7: 8 iter × 2 mma_ABt × 1 mfma_1616x32 per call = 16 MFMA)
            // Each iter covers 256 K-cols (2 tiles × 128 cols).
            //   Iter 0: kv_0_wide at col 0, kv_1_wide at col 128
            //   Iter 1: kv_0_wide at col 256, kv_1_wide at col 384
            //
            // Q wide tiles q_0_wide, q_1_wide are rt_16x128_s (8 VGPRs per lane).
            // We use q_nope range (32 VGPRs) as 2 iters × 16 VGPRs (q_0 8 + q_1 8).
            // Each iter reloads q_0_wide/q_1_wide via lds_2_gpr_wide.
            // -----------------------------------------------------------------
            constexpr uint32_t num_nope_iter = T::kQkNopeHeadDim / (T::kBlockN_Wide * 2);  // 512/(128*2)=2
            ckt::static_for<0, num_nope_iter, 1>{}([&](auto idx) {
                constexpr uint32_t reg_start = idx.value * 16 + k_q_nope_begin;
                // Q wide ranges: 8 VGPRs per q-tile, split into 2 row-halves (4 VGPRs each row-half
                // for internal rt_16x128_s iteration — the base tile has 16 rows, covered by 1
                // row-half at kTileM=16).
                using q_wide_range_0 = hkdart::split_many_t<
                    hkdart::type_list<hkdart::range<reg_start, reg_start + 7>>, 8>;
                using q_wide_range_1 = hkdart::split_many_t<
                    hkdart::type_list<hkdart::range<reg_start + 8, reg_start + 15>>, 8>;
                hk::art<q_t, T::kTileM, T::kBlockN_Wide, hk::row_l, hk::rt_16x128_s, q_wide_range_0> q_0_wide;
                hk::art<q_t, T::kTileM, T::kBlockN_Wide, hk::row_l, hk::rt_16x128_s, q_wide_range_1> q_1_wide;

                // Reload Q for this iter's wide view.
                // LDS Q-block covers 64 cols per block; 128-wide span = 2 blocks starting at
                // (idx.value * 256) or (idx.value * 256 + 128).
                constexpr uint32_t kQColBase_0 = idx.value * 256;
                constexpr uint32_t kQColBase_1 = idx.value * 256 + 128;
                // V9 experiment: NO reload — Q narrow data at reg_start..reg_start+15 is
                // already loaded by q_manager.load_q_to_gpr at function start. The q_wide
                // range types below reinterpret those 16 VGPRs as 2 rt_16x128_s tiles.
                // Relies on MFMA accepting narrow-interleaved lane layout.

                // V9 K load via load_k_wide_to_gpr. Each call writes 8 VGPRs (4 × ds_read_b64).
                // Produces narrow-interleaved lane layout (lane owns cols {0-7,32-39,64-71,96-103}).
                kv_manager.template load_k_wide_to_gpr<0,  kQColBase_0>(kv_0_wide, p_lds_kv_curr);
                kv_manager.template load_k_wide_to_gpr<16, kQColBase_0>(kv_0_wide, p_lds_kv_curr);
                kv_manager.template load_k_wide_to_gpr<0,  kQColBase_1>(kv_1_wide, p_lds_kv_curr);
                kv_manager.template load_k_wide_to_gpr<16, kQColBase_1>(kv_1_wide, p_lds_kv_curr);

                asm volatile("s_waitcnt lgkmcnt(2)");
                if constexpr(idx.value == 0)
                {
                    hk::mma_ABt(p_comp, kv_0_wide, q_0_wide);
                    __builtin_amdgcn_s_setprio(15);
                }
                else
                {
                    hk::mma_ABt(p_comp, kv_0_wide, q_0_wide, p_comp);
                }
                asm volatile("s_waitcnt lgkmcnt(0)");
                hk::mma_ABt(p_comp, kv_1_wide, q_1_wide, p_comp);
            });

            // RoPE GEMM (unchanged from v7, narrow kv_0/kv_1 rt_16x32_s).
            ...
            ckt::static_for<0, num_rope_iter, 1>{}([&](auto idx) {
                ...
                using q_range_0 =
                    hkdart::split_many_t<hkdart::type_list<hkdart::range<reg_start, reg_start + 1>>, 2>;
            ...
            // PV GEMM (unchanged from v7 — K-reduction axis = kBlockN = 32, cannot widen).
...
void mla_v32_fwd_decode_h32_fp8_fp8_v9(torch::Tensor& query,
                                        ...
                                        torch::Tensor& final_output)
{
    ...
    kn_mla_v32_fwd_decode_h32_fp8_fp8_v9<Traits>
        <<<grid, Traits::kNumThreads, lds_size, stream>>>(params);
}

void hk_mi3xx_mla_v32_fwd_decode_h32_fp8_fp8_v9(torch::Tensor& query,
                                                 torch::Tensor& kv_buffer,
                                                 const torch::Tensor& qo_indptr,
                                                 const torch::Tensor& kv_indptr,
                                                 const torch::Tensor& kv_page_indices,
                                                 ...
                                                 torch::Tensor& final_output)
{
    ...
    {
        using Traits = HkMlaDecodeFwdTraitsH32V9<hk::fp8e4m3, hk::fp8e4m3, hk::bf16, 32>;
        mla_v32_fwd_decode_h32_fp8_fp8_v9<Traits>(query,
                                                   kv_buffer,
                                                   qo_indptr,
                                                   kv_indptr,
                                                   kv_page_indices,
    ...
    {
        TORCH_CHECK(false,
                    "hk_mi3xx_mla_v32_fwd_decode_h32_fp8_fp8_v9 doesn't support q type ",
