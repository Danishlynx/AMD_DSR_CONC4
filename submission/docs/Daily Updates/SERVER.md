7756932064 just use this doirect;ly



# DSR1 CONC=4 — SERVER COMMANDS & INFRASTRUCTURE (merged: INFRA + SERVER_MAP)

**Last updated**: 2026-05-04 ~13:00 UTC (May 04 truth banner + post-purge container layout; legacy content preserved verbatim below)

> 🚫 **MAY 04 KIMBOCHEN-ONLY DIRECTIVE (read first):** only `dsr1_benchmark perf` (kimbochen wrapper) produces authoritative DSR1 numbers. Every "thr/GPU" or "TPOT" cited below WITHOUT explicit kimbochen-4-iter qualifier is a fluke. The legacy "RE.1 INT4 AR 1360 thr/GPU" line below was Apr 22 informal harness — under kimbochen the same stack gives ~1268/1 of 4 gates. Do not cite legacy non-kimbochen numbers as targets, ceilings, or regress baselines.

> 🎯 **MAY 04 TRUTH BANNER (post-snapshot-purge):**
>
> **Live container**: `dsr1_fresh` (id `76af9252fb3b`) on `amd-gpu` host, GPUs 0-3 (renderD128/136/144/152), shm 32GB, mounts `/projects/teamA/` + HF cache. Created May 04 from sole live image `rocm/atom-dev:dsr1_apr30_phase11_v3_2of4` (sha c58cf2ce4512). **Containers `re4c_v10` and `dsr1_prof` REMOVED.** Kimi containers (`kimi_prof`, `kimi_baseline`) untouched on GPUs 4-7.
>
> **Live source state in `dsr1_fresh`** (inherited from GOLD snapshot):
> - `/app/ATOM/atom/model_ops/rejection_sampler.py`: BASELINE TOP_N=10 / DELTA=0.6 (Lever G TOP_N=12 NOT yet applied as a persistent edit; if applied during a session, backup at `/tmp/rejection_sampler.py.preTrack1_G_top12_<TS>`).
> - `/app/aiter-test/aiter/configs/model_configs/dsv3_fp4_tuned_fmoe.csv`: pristine (md5 `7ca7f861b300628922a9e926912d2cc3`, 47 rows). F4 retune reverted.
> - `/app/aiter-test/aiter/ops/flydsl/moe_kernels.py:72`: t16 registry expansion KEPT for FP4 stage1 (architectural consistency with stage2; t16 never wins at our shape but harmless).
> - Phase 11 per-phase MTP env-gate ON (TRT-LLM port).
> - INT4 QR + MSCCLPP + dual-stream MoE 1024 + GPU_MAX_HW_QUEUES=5 active.
> - Layer 61 / lm_head / mlp.gate stay BF16 per Quark `exclude` rule.
>
> **Sole shipping snapshot**: `rocm/atom-dev:dsr1_apr30_phase11_v3_2of4` sha `c58cf2ce4512` + alias `locked/dsr1:phase11_v3_2of4`. Authoritative kimbochen 4-iter: **TPOT 5.641 / GSM 0.9318 PASS / Intvty 177.26 PASS / Tput 1449 / E2E 6210 = 2/4 gates**.
>
> **Provisional WIN (kimbochen-pending)**: Lever G TOP_N=12 dev-bench TPOT 5.5141 (−0.200 ms), GSM 0.9401. Apply via single-line edit at `rejection_sampler.py:22`; not in source by default in `dsr1_fresh`.
>
> See [Plan.md](Plan.md) for the May 04 forward-lever plan, [REPRODUCE.md](REPRODUCE.md) for `dsr1_fresh` boot recipes, and [OFFICIAL_HARNESS.md](OFFICIAL_HARNESS.md) for the bench contract.

---

## 🐳 May 04 container map (post-purge)

| Container | Image | GPUs | Status | Use |
|---|---|---|---|---|
| **`dsr1_fresh`** | `rocm/atom-dev:dsr1_apr30_phase11_v3_2of4` | 0-3 (renderD128/136/144/152) | Live | **Sole DSR1 work container** |
| `kimi_prof` | `locked/kimi:a1_routing_validated` | 4-7 | Live | Kimi track |
| `kimi_baseline` | `fe87c4dc1792` | 4-7 | Live | Kimi track |

Removed May 04: `re4c_v10` (exited 22h, rm), `dsr1_prof` (running 19h, stopped+rm).

### `dsr1_fresh` exec patterns
```bash
# Boot DSR1 server
docker exec -d dsr1_fresh bash /tmp/boot_phase11_v3_memfix.sh

# Watch boot log
docker exec dsr1_fresh tail -f /tmp/phase11_memfix_boot_<TS>.log

# Health check
docker exec dsr1_fresh curl -s http://0.0.0.0:8890/v1/models

# Kill server CLEANLY (per-PID; NEVER pkill -f from inside the SSH chain — can self-kill)
docker exec dsr1_fresh bash -c 'pgrep -fa openai_server'
# Then `docker exec dsr1_fresh kill -9 <each PID>` individually

# Run kimbochen 4-iter
docker exec dsr1_fresh bash /tmp/dsr1_benchmark_4iter.sh
# Take median(iter2, iter3, iter4)
```

### Critical operational rules
- **kimbochen wrapper REQUIRES** `HF_HOME=/hf_cache HUGGINGFACE_HUB_CACHE=/hf_cache/hub HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1` exports or it hits `/root/.cache/huggingface` PermissionError.
- **8 small-prompt warmup curls** (max_tokens=50) BEFORE bench. Partial-warmup taints first iter (was the Apr 26 mismeasurement source).
- **AITER CSV merger globs `*.csv*` in `model_configs/`** at boot. NEVER leave backup files (`.preTrack1_*`, `.bak`) inside `aiter/configs/model_configs/` — move to `/tmp/` or `/projects/teamA/danish/<task>_evidence/`.
- **pkill BEFORE docker restart**, not after. HIP S-state orphans survive container restart in different PID namespace; can't be killed from new container without sudo or `rocm-smi --gpureset`.
- **Lever G is a single-line edit** at `rejection_sampler.py:22` (`PHASE_RELAXED_TOP_N = 10` → `12`); do NOT touch line 23 (`PHASE_STRICT_TOP_N = 8`) — that protects outside-`<think>` GSM accuracy.

---

## 🚨 LEGACY MAY 02 TRUTH BANNER (preserved for archeology):
>
> The legacy "Current production config: RE.1 INT4 AR (1360 thr/GPU)" line below is from **2026-04-22 informal harness** (no `--use-chat-template`) and is wrong as a current-state claim. The same RE.1 stack measured under official kimbochen harness gives **1268 thr/GPU = 1/4 gates** (because reasoning mode activates and is ~15% slower per token).
>
> **May 02 production state (now obsolete — `re4c_v10` removed):**
> - Live container WAS: `re4c_v10` (REMOVED May 04)
> - Live source WAS: `/app/ATOM/atom/spec_decode/eagle.py` reverted to 319-line clean baseline (W3.2 v4 cudagraph drafter REVERTED at +0.317 ms regression)
> - Live AITER cherry-pick WAS: PR #2927 (`f64ebb6`) on branch `session17_fp8_sq8_mla` — 3-site `gfx942 → gfx942 OR gfx950` gate flip (branch deleted with snapshot purge)
> - Live FlyDSL: 0.1.3.1 (0.1.4.2 upgrade KILLED May 02, GSM 0.9219)
> - Live envs: gold stack (RELAXED_TOP_N=8, INT4 AR, MSCCLPP, dual-stream MoE 1024) + Phase 11 per-phase MTP env-gate ON

---

## 🏆 [LEGACY 2026-04-22] Production config: RE.1 INT4 AR (1360 thr/GPU informal harness — see banner above)
