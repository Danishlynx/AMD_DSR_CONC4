...
**Last updated**: 2026-05-04 ~13:00 UTC — May 04 truth banner + post-purge container recipe.

> 🚫 **MAY 04 KIMBOCHEN-ONLY DIRECTIVE (read first):** only `dsr1_benchmark perf` (kimbochen wrapper: chat-template + reasoning + 8-warmup ladder + GSM8K validation) produces authoritative DSR1 numbers. Dev-bench / direct `benchmark_serving` / informal harness numbers are FLUKES. Lever WIN = kimbochen 4-iter median(2,3,4) on the GOLD snapshot or a successor. The only authoritative DSR1 baseline is GOLD `dsr1_apr30_phase11_v3_2of4` (sha c58cf2ce4512) kimbochen 4-iter: **TPOT 5.641 / GSM PASS / Intvty 178 / 2-of-4**.

> 🎯 **MAY 04 TRUTH BANNER (post-snapshot-purge):**
>
> **Sole live DSR1 image**: `rocm/atom-dev:dsr1_apr30_phase11_v3_2of4` sha `c58cf2ce4512` (+ alias `locked/dsr1:phase11_v3_2of4`). All other DSR1 snapshots PURGED May 04 — old May 01 P1, Apr 29 v6c, Apr 23 Champion, Apr 24-30 trial state ALL deleted.
>
> **Sole live work container**: `dsr1_fresh` (id `76af9252fb3b`, GPUs 0-3) — created May 04 from GOLD. `re4c_v10`, `dsr1_prof` removed. Use `dsr1_fresh` for all reproduction.
>
> **Authoritative kimbochen baseline (the only number to compare against)**: GOLD 4-iter median **TPOT 5.641 / GSM 0.9318 PASS / Intvty 177.26 PASS / Tput 1449 / E2E 6210 = 2/4 gates**. Need −1.04 ms TPOT to clear E2E gate.
>
> **Provisional WIN (dev-bench, kimbochen-pending)**: Lever G `PHASE_RELAXED_TOP_N 10→12` — dev-bench 4-iter med(2,3,4) **TPOT 5.5141 ms (−0.200 ms)**, GSM 3-iter med 0.9401 PASS. Source edit lives in `dsr1_fresh` only; NOT yet kimbochen-validated, NOT yet snapshotted.
>
> **May 03-04 Phase A/B levers — all NEUTRAL/no-win at noise floor** (architectural insights documented in MASTER.md and Plan.md): F1 force run_1stage / F1 v2 ASM CSV / F4 retune / F2 tile_m=16 stage1 / F7 moe_fused_gate / Lever Z _fp4-suffix-everything (GSM kill).
>
> See [Plan.md](Plan.md) for the May 04 forward-lever plan, [OFFICIAL_HARNESS.md](OFFICIAL_HARNESS.md) for the harness contract, and [SNAPSHOT_INVENTORY_apr24.md](SNAPSHOT_INVENTORY_apr24.md) for the live snapshot ranking.

---

## ⚙️ May 04 — `dsr1_fresh` recipe (reproduce GOLD baseline + apply Lever G)

### Container creation (already done, kept for reference)
```bash
docker run -d --name dsr1_fresh \
  --network bridge --shm-size 32g \
  --device /dev/kfd \
  --device /dev/dri/renderD128 \
  --device /dev/dri/renderD136 \
  --device /dev/dri/renderD144 \
  --device /dev/dri/renderD152 \
  -v /projects/teamA/hf_cache:/tmp/.cache/huggingface \
  -v /projects/teamA/:/projects/teamA/ \
  rocm/atom-dev:dsr1_apr30_phase11_v3_2of4 \
  sleep infinity
```
Note: `--security-opt`, `--privileged`, `--cap-add`, `--runtime` are BLOCKED by team docker wrapper — omit. Default apparmor applies.

### Reproduce GOLD baseline (kimbochen 4-iter, the authoritative bench)
```bash
docker exec -d dsr1_fresh bash /tmp/boot_phase11_v3_memfix.sh
# Wait ~12 min for cudagraph capture
# 8 small-prompt warmup curls (CRITICAL — partial-warmup taints first iter)
docker exec dsr1_fresh bash -c '
  for i in 1 2 3 4 5 6 7 8; do
    curl -s -X POST http://0.0.0.0:8890/v1/completions \
      -H "Content-Type: application/json" \
      -d "{\"model\":\"amd/DeepSeek-R1-0528-MXFP4\",\"prompt\":\"What is 2+2?\",\"max_tokens\":50}" > /dev/null
  done
'
# Run kimbochen 4-iter
docker exec dsr1_fresh bash /tmp/dsr1_benchmark_4iter.sh
# Take median(iter2, iter3, iter4); discard iter1 (cold)
# Expected GOLD: TPOT 5.641, GSM 0.9318, Intvty 177.26, 2/4 gates
```

### Apply Lever G (TOP_N=12) and re-bench
```bash
docker exec dsr1_fresh bash -c '
  cp /app/ATOM/atom/model_ops/rejection_sampler.py /tmp/rejection_sampler.py.preTrack1_G_top12_$(date +%H%M%S)
  sed -i "s/^PHASE_RELAXED_TOP_N = 10/PHASE_RELAXED_TOP_N = 12/" /app/ATOM/atom/model_ops/rejection_sampler.py
  grep -n "PHASE_RELAXED_TOP_N\|PHASE_STRICT_TOP_N" /app/ATOM/atom/model_ops/rejection_sampler.py
'
# Kill server cleanly (per-PID, not pkill which can self-kill via SSH parent)
docker exec dsr1_fresh bash -c 'pgrep -fa openai_server | head'
# Then `kill -9 <each PID>`, NOT `pkill -f openai_server`
# Reboot + re-run kimbochen 4-iter
# If TPOT_med ≤ 5.55 AND GSM_med ≥ 0.9325 → WIN, snapshot
docker commit dsr1_fresh rocm/atom-dev:dsr1_may04_leverG_kimbochen
```

### Revert Lever G (rollback)
```bash
docker exec dsr1_fresh bash -c '
  ls /tmp/rejection_sampler.py.preTrack1_G_top12_*
  cp /tmp/rejection_sampler.py.preTrack1_G_top12_<TS> /app/ATOM/atom/model_ops/rejection_sampler.py
'
```

---

## 🚨 LEGACY MAY 02 BANNER (preserved for archeology)
>
> **Current state**: 1/4 official-harness gates @ TPOT 5.777 ms post PR #2927 cherry-pick (measured on `re4c_v10`, since removed). Iter1 alone hits 2/4 (Tput at 1500 gate-edge). GSM iter1 0.9303 PASS, GSM_med 0.9280 (within ±0.003 noise of 0.93 gate).
>
> **W3.2 drafter cudagraph campaign closed at +0.317 ms regression (May 02).** v4 worked end-to-end (24 captures, 0 SIGABRTs, GSM 0.9310 PASS) but EV thesis was empirically wrong by 5×. Reverted to clean baseline (319-line eagle.py).
>
> **AITER PR #2927 cherry-pick (qh128 gfx950 gate flip)** lived in tree on branch `session17_fp8_sq8_mla` as `f64ebb6`. Real but tiny (−0.033 ms TPOT, within noise floor). NO new snapshot. Branch deleted with snapshot purge.
>
> **MTP=4 retest with PR #2927 still crashes** at `asm_mla.cu:308` — ATOM `attention_mla.py:_forward_decode` pre-folds nhead=128→16 BEFORE aiter call, so PR #2927's gate-flip never reaches the dispatcher. ATOM patch needed (~2h, not done).

---

**May 01 status (preserved)**: AITER-Absorption + R2-v2 Persistent plan delivered ΔTPOT=0 across P2/P3/P4 (each killed for distinct architectural reasons). Canonical baseline: **TPOT 5.894 ms / GSM_med 0.9310 / 2-of-4** at snapshot `rocm/atom-dev:dsr1_may01_clean_baseline_5894` (sha `b6f8c9d03206`). P1 working state at `dsr1_may01_p1_aiter_cherry` (sha `7630c22aee40`). See [MASTER.md](MASTER.md) top entry for full May-01 details.

---

## 🚦 May 01 — Snapshot rollback chain
...
| `rocm/atom-dev:dsr1_may01_clean_baseline_5894` | `b6f8c9d03206` | 5.894 | Phase 0 anchor (post-cache-wipe clean) |
| `rocm/atom-dev:dsr1_apr30_phase11_v3_2of4` | `c58cf2ce4512` | 5.641 (Apr 30 KEEP) | Plan rollback target |
| `locked/dsr1:phase11_v3_2of4` | `c58cf2ce4512` | (mirror) | Locked alias of plan rollback |

To reproduce P1 state cleanly:

```bash
# 1. From host:
docker stop re4c_v10 && docker rm re4c_v10
docker run -d --name re4c_v10 \
  ...
  -v /projects/teamA/hf_cache:/tmp/.cache/huggingface \
  rocm/atom-dev:dsr1_may01_p1_aiter_cherry sleep infinity

# 2. Boot Phase 11 v3 (cherry-pick gates default OFF -> perf-neutral):
docker exec -d re4c_v10 bash /tmp/boot_phase11_per_phase_mtp.sh

# 3. Wait ~10 min for cudagraph captures, then:
docker exec re4c_v10 bash /tmp/official_bench_3x.sh
# Expect: TPOT_med ~5.86 ms, GSM_med (9 runs) ~0.93, 2/4 gates
```

---

## 🛑 May 01 — What WAS NOT merged into production

Three architectural carries were tried and killed. Production CSV `dsv3_fp4_tuned_fmoe.csv` line 42 is unchanged from Phase 0 pristine (`flydsl_moe2_afp4_wfp4_bf16_t32x128x256_atomic_bnt2`). Cherry-pick of #2823 stays in branch as inert code (env gate `ATOM_AITER_FUSED_AR_RMSNORM_QUANT` default OFF; consumer side never wired because no DSR1 integration point exists).

Preserved evidence in container `/tmp/aiter_backups/`:
- `dsv3_fp4_tuned_fmoe.csv.original` (pristine restore source)
- `p3a_fp4_tune_evidence.csv` + `p3a_fp4_tune.log` (1.6 MB — proves FlyDSL config space exhausted at M=16)
- `p4_r2v2_evidence.tar.gz` (5.3 KB — three R2-v2 v0.x kernel variants + microbench harness, proves grid reduction makes kernel SLOWER)

---

## 🛠️ R2-C M1 KERNEL BUILD RECIPE — compile + launch + no crash (Apr 30 ~12:30 UTC)

### Hot shape (from R2-A capture)
```
M=4, H=7168, intermediate=512, num_experts=257, topk=9
hidden_dtype=bf16, quant_w_dtype=fp4x2, quant_a_dtype=fp4x2, QuantType.per_1x32
Dispatched: [fused_moe] using 2stage default (CK ck2stages fallback, NOT tuned FlyDSL)
```

### T0 baseline (from R2-B microbench, 1000 iters, single GPU)
```
T0 = 71.88 µs/call
Compute: 0.85% of FP4 peak (11.0/1300 TFLOPS)
HBM:     13.82% of bandwidth peak (732/5300 GB/s)
```

### Source files
| File | Path |
|---|---|
| Kernel skeleton (.cu) | `/tmp/r2_smallm_moe_gemm2.cu` |
| Build script (.sh) | `/tmp/r2_build.sh` |
| ctypes harness (.py) | `/tmp/r2_harness.py` |
| Snapshot tensors (.pt) | `/tmp/r2_snapshot/` (saved by `ATOM_R2_SNAPSHOT_TENSORS=1` patch) |
| Microbench script (.py) | `/tmp/r2b_microbench.py` |
| Snapshot patch script (.py) | `/tmp/r2b_snapshot_patch.py` |

Mirrored on laptop at `patches/scripts/r2_kernel/`.

### Build command
```bash
hipcc --offload-arch=gfx950 -O1 -fPIC -shared -std=c++17 \
  -DHIP_ARCH_GFX950=1 -I/opt/rocm/include \
  -o /tmp/r2_smallm_moe_gemm2.so /tmp/r2_smallm_moe_gemm2.cu
```
- `-O1` is critical — `-O3` segfaults LLVM machine scheduler on `mfma_scale` interleave (Apr 29 R1 OOB lesson).
- ROCm 7.2.2, clang 22.0.0git, HIP 7.2.53211 (verified working).

### Launch command (M1 sanity)
```bash
HOME=/tmp HF_HOME=/tmp/.cache/huggingface HUGGINGFACE_HUB_CACHE=/tmp/.cache/huggingface/hub \
TRANSFORMERS_CACHE=/tmp/.cache/huggingface/hub HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 \
HIP_VISIBLE_DEVICES=0 \
python3 /tmp/r2_harness.py
```

### M1 success criteria (proven Apr 30 ~12:30 UTC)
- `hipError_t = 0` returned by launcher
- `cudaDeviceSynchronize` OK with no HSA exception
- 20832-byte .so emitted with gfx950 ISA

### Architectural decisions locked
- **MFMA wrapper:** templated on `CBSZ=4/ABID=0/BLGP=4/OPSEL_A=0/SCALE_A=0/SCALE_B=0` constexpr params (LLVM intrinsic requires compile-time constants).
- **Tile:** M=4 valid (HW-fixed pad-to-16 in MFMA), N=32, K=128. 56 K-iters per expert (7168/128).
- **Workgroup:** 64 threads (1 wave). LDS budget per WG: ~6.2 KB.
- **Atomic output:** `global_atomic_pk_add_bf16` for cross-expert reduction.
- **LDS load:** `ds_read_b64_tr_b4` for FP4 k-interleaved transpose load.

### Pending milestones
- **M2** Numerics parity vs `aiter.fused_moe(...)` reference output (target `err_ratio ≤ 1e-4`)
- **M3** Perf tune (target ≤47.92 µs/call = 1.5× T0 speedup)
- **M4** AITER dispatcher env-gated integration (`AITER_USE_R2_SMALLM=1` + exact-shape guard)
- **M5** Official kimbochen bench under Phase 11 v3 + R2 (KEEP threshold ≥ −0.30 ms TPOT)

---

## 🎯 PHASE 11 PER-PHASE MTP v3 RECIPE — 2-of-4 GATES (Apr 30 ~07:55 UTC)

### Result (single-iter, official kimbochen-equivalent harness)

| Metric | Value | Gate | Verdict |
|---|---:|---:|---|
| GSM8K_med (N=3) | **0.9318** | ≥ 0.93 | ✅ PASS (margin +0.0018) |
...
GSM8K runs: 0.9356 / 0.9318 / 0.9318.

### Source patches (env-gated NULL-OP at default; merge-clean)

Three files patched. All have `.pre_per_phase_mtp` backups in container.

#### 1. `/app/ATOM/atom/utils/envs.py`
After existing `ATOM_ENABLE_RELAXED_MTP` lambda, add:
```python
"ATOM_ENABLE_PER_PHASE_RELAXED_MTP": lambda: os.getenv(
    "ATOM_ENABLE_PER_PHASE_RELAXED_MTP", "0").lower() == "1",
```

#### 2. `/app/ATOM/atom/model_engine/model_runner.py`
After `self.max_bs = self.config.max_num_seqs` (line ~1011) — allocate per-seq phase tensor and register with sampler module:
```python
...
After `is_prefill = batch.total_tokens_num_prefill > 0` in `prepare_inputs()` — reset prefill-slot phases:
```python
...
#### 3. `/app/ATOM/atom/model_ops/rejection_sampler.py`
Add module-level globals + setter:
```python
...
PHASE_STRICT_TOP_N = 8     # baseline-equivalent (outside thinking)
PHASE_RELAXED_DELTA = 0.6
THINK_TOKEN_ID = 128798    # DSR1 R1 <think>
ENDTHINK_TOKEN_ID = 128799 # DSR1 R1 </think>

_spec_phase_tensor = None
...
Add new Triton kernel `rejection_phased_sample_kernel` (full body in `bench_results/apr30_phase11_per_phase_mtp_v3_KEEP_2of4/README.md`). Key shape: `phase_ptr` is `[batch_size]` int8. Kernel loads phase as int32 inside, casts back to int8 on store.

In `rejection_sample()` — add phased dispatch path BEFORE existing `if RELAXED_TOP_N <= 1:` branch:
```python
if ATOM_ENABLE_PER_PHASE_RELAXED_MTP and _spec_phase_tensor is not None:
    ...
    rejection_phased_sample_kernel[(batch_size,)](
        output_token_ids, num_bonus_tokens, cu_num_draft_tokens,
        draft_token_ids, topn_ids, bonus_token_ids, _phase_slice,
        num_spec_steps, PHASE_RELAXED_TOP_N,
        PHASE_STRICT_TOP_N, PHASE_RELAXED_TOP_N,
        THINK_TOKEN_ID, ENDTHINK_TOKEN_ID,
        num_warps=1,
    )
...
Same as L0-v2 baseline (gold+v6c + `ATOM_CUDAGRAPH_MODE=FULL_DECODE_ONLY`) + add:
```bash
export ATOM_ENABLE_PER_PHASE_RELAXED_MTP=1
```

Container ref: `/tmp/boot_phase11_per_phase_mtp.sh`.

### Bench

Standard `/tmp/official_bench_n3.sh` (kimbochen-equivalent harness via `atom.benchmarks.benchmark_serving`, byte-equivalent to `kimbochen/bench_serving`'s `benchmark_serving.py` — verified Apr 30 against canonical `dsr1_benchmark.cpp`). Args: `--num-prompts 40 --max-concurrency 4 --random-input-len 8192 --random-output-len 1024 --random-range-ratio 1.0 --num-warmups 8 --use-chat-template --request-rate inf --ignore-eos`.

### Boot timing
~13 min cold-start (similar to L0-v2 baseline; ~30s longer due to additional Triton kernel JIT compile). All 6 cudagraphs at bs={1,2,4,8,16,32} max_q_len=4 captured cleanly in 1.5s. No HSA exceptions, no AssertionErrors, no torch.compile recompiles.

### Backup chain (per Backup.md)
- Container source: `.pre_per_phase_mtp` backups for all 3 patched files
- Host bind: `/projects/teamA/danish/apr30_evidence/phase11_per_phase_mtp_v3_KEEP_2of4/` (full bench artifacts + boot log)
- Laptop: `bench_results/apr30_phase11_per_phase_mtp_v3_KEEP_2of4/` (README + evidence.tgz with bench artifacts + boot log)
- Git: branch `session17_fp8_sq8_mla` commit `5ca90c5` (pushed to origin)

### Distance to 4/4
TPOT 5.641 → ~4.54 ms = **−1.10 ms more** to clear E2E gate. Stack v6e v2.3 (BufferRing + zeros init) + R2 small-M MoE GEMM next.

---

# 🟢 CANONICAL kimbochen-VALIDATED RECIPE (2026-04-29 ~04:00 IST) — gold+v6c

> **Trust rule**: numbers in this block were measured under `kimbochen/dsr1_benchmark.cpp` ONLY. Anything elsewhere in this file labelled with the wrong-informal-harness banner OR predating Apr 27 ~06:15 IST is **suspect** and must not be cited without re-measurement.

## Best validated result

3-iter median (Apr 29 03:00 IST, kimbochen-official harness, fresh boot):

| Metric | Median | Best iter | Gate (CONC=4) | Status |
|---|---:|---:|---:|:---:|
| GSM8K (num_fewshot=3 flexible) | **0.9340** | 0.9416 | ≥0.93 | ✅ PASS |
| TPOT median (ms) | **6.080** | 5.925 | (implied ≤6.06 from Intvty) | ❌ -0.02 ms gap (within noise) |
| Median E2E (ms) | **6743** | 6584 | ≤5000 | ❌ -1743 ms gap |
| Tput / GPU (TP=4 → /4) | **1403** | 1424 | ≥1500 | ❌ -97 (-6.5%) |
| Interactivity | **164.48** | 168.78 | ≥165 | ❌ -0.52 (within noise; iter3 alone PASSES at 168.78) |

**1-2/4 gates** (GSM8K + Intvty marginal). Best single iter (iter3 of run1) = **2/4 gates** (GSM8K + Intvty PASS at 168.78).

**Re-bench (3 more iters Apr 29 03:13 IST same stack):** TPOT 6.111 / 6.357 / 6.376 → median 6.357. Combined 6-iter median ≈ 6.20 ms. Variance ±0.25 ms attributable to host contention.

## Snapshot

`rocm/atom-dev:dsr1_apr29_gold_v6c_2of4` — sha `4911df6937b152` (487 GB) — also tagged `locked/dsr1:gold_v6c_2of4`.

## Source patches (only this stack — older deltas archived below)

**Single source change**: `/app/ATOM/atom/model_ops/layernorm.py` — env-gated v6c MXFP4 fused-AR-RMSNorm dispatch.

```python
# BEFORE the existing fused_allreduce dispatch:
import os as _os_l2v6
from aiter.dist.communication_op import (
    tensor_model_parallel_fused_allreduce_rmsnorm,
    tensor_model_parallel_fused_allreduce_rmsnorm_mxfp4_quant,
)

# Inside RMSNorm.forward, replace the existing fused_allreduce branch with:
if self.fused_allreduce and self.tp_size > 1:
    assert residual is not None
    _CUSTOM_AR_MAX = 8192 * 8192   # custom_ar size limit (decode window)
    _inp_bytes = x.numel() * x.element_size()
    _use_mxfp4 = (
        _os_l2v6.environ.get("ATOM_USE_POST_ATTN_RMSNORM_MXFP4_FUSION", "0") == "1"
        and _inp_bytes <= _CUSTOM_AR_MAX
    )
    if _use_mxfp4:
...
The size-guard is mandatory: prefill batches (token=32768) exceed the 8192·8192-byte `should_custom_ar` window in `aiter/dist/device_communicators/custom_all_reduce.py:435`; without it, prefill triggers an internal assertion. Decode batches (token≤32) always fit.

Patch script (idempotent): `/projects/teamA/danish/apr28_evidence/patches/l2v6c_patch.py` (host bind-mount).

**No other source changes vs the upstream Apr 26 stack.** No deepseek_v2.py modifications. No model_runner.py modifications. v6d/v6e (the prequant-stash variant) is NOT in this snapshot — that attempt failed with cudagraph-replay incompatibility (Python dict mutation inside captured graph). Reverted.

## Boot environment (verbatim) — gold+v6c

```bash
LOGFILE=/tmp/gold_v6c_boot_$(date +%H%M%S).log

# Reset perf-determinism (CRITICAL — let GPU boost 2100 → 2396 MHz)
...
# Collectives — RCCL ROCm 7.1+ knobs (proven +6.8% throughput on Apr 22)
...
# QuickReduce INT4 AR (RE.1 lock — proven +6.8% throughput; verified PASS GSM8K on DSR1)
...
# v6c WIN — gold+v6c is THIS stack with v6c env-gated ON
...
**RELAXED_TOP_N must remain at 8** (the canonical baseline). Setting `=9` was the Apr 26 informal-harness "win" but **fails GSM8K under the kimbochen harness** (median 0.9287, gate 0.93). DO NOT change.

Boot script saved: `/tmp/boot_gold_v6c.sh` (in container) and `/projects/teamA/danish/apr28_evidence/scripts/boot_gold_v6c.sh` (host bind-mount).

## Bench — kimbochen official binary (only valid harness)

```bash
# Binary path on host (compiled from kimbochen/dsr1_benchmark.cpp)
BENCH=/projects/teamA/danish/repos/amdgpu_bounty_optimization/dsr1-fp4-atom-mtp-mi355x/dsr1_benchmark

# Required env for the binary
export MODEL=amd/DeepSeek-R1-0528-MXFP4
export PORT=8890
export TP=4                         # CRITICAL — binary defaults to /8 in tput calc; we override to /4
export CONC=4
export ISL=8192
export OSL=1024
...
# Optional warmup (binary already does --num-warmups 8 internally; this is belt-and-suspenders)
for i in 1 2 3 4 5 6 7 8; do
  curl -sf -X POST http://0.0.0.0:8890/v1/completions \
    -H "Content-Type: application/json" \
    -d "{\"model\":\"amd/DeepSeek-R1-0528-MXFP4\",\"prompt\":\"hi $i\",\"max_tokens\":8,\"temperature\":0}" > /dev/null
done

# 3-iter run
RUNDIR=/tmp/gold_v6c_bench_$(date +%H%M%S)
mkdir -p $RUNDIR
...
for IT in 1 2 3; do
  python3 -c "
import json
d=json.load(open('$RUNDIR/result_iter${IT}.json'))
tpg = d.get('total_token_throughput', 0) / 4.0    # divide by TP=4
print(f'iter${IT}: TPOT={d[\"median_tpot_ms\"]:.3f}ms E2E={d[\"median_e2el_ms\"]:.0f}ms Tput/GPU={tpg:.0f} Intvty={1000/d[\"median_tpot_ms\"]:.2f}')
"
done
```

The binary internally:
1. Runs `lm_eval --tasks gsm8k --num_fewshot 3 --model_args num_concurrent=65,max_retries=1,tokenized_requests=False`. Gate ≥0.93. **If FAIL → perf phase is SKIPPED.**
2. Clones `kimbochen/bench_serving` to `/tmp/bmk-<ts>` and runs perf with `--random-range-ratio 1.0 --use-chat-template --num-warmups 8 --num-prompts 40 --max-concurrency 4`.
3. Computes `tput_per_gpu = total_token_throughput / 8.0` (binary hard-codes /8 for TP=8). For our TP=4 you must **divide the binary's tput_per_gpu output by 0.5**, OR (recommended) compute it yourself from `total_token_throughput / 4.0` as shown in the parser above.

The binary requires internet (clones the bench_serving repo each invocation). If sandboxed, pre-clone once and patch out the git step.

## Reproduce in 3 commands (assuming the snapshot is on the host registry)

```bash
# 1) Spawn container from kimbochen-validated snapshot
docker run -d --name dsr1_repro \
  ...
  -v /projects/teamA/danish/repos/amdgpu_bounty_optimization:/projects/teamA/danish/repos/amdgpu_bounty_optimization \
  -e HIP_VISIBLE_DEVICES=0,1,2,3 \
  rocm/atom-dev:dsr1_apr29_gold_v6c_2of4
# (or: locked/dsr1:gold_v6c_2of4)

# 2) Boot (~12 min for cudagraph capture)
docker exec -d dsr1_repro bash /tmp/boot_gold_v6c.sh
# tail $(cat /tmp/last_boot_log.txt) until "Application startup complete"

# 3) Bench
docker exec -d dsr1_repro bash /tmp/bench_r1a_official.sh
# results in /tmp/r1a_official_<HHMMSS>/result_iter{1,2,3}.json
```

Evidence on host bind: `/projects/teamA/danish/apr29_evidence/{bench_results,patches,scripts}/`.

## What's in this stack vs A27 baseline (Apr 27 kimbochen-official 1/4)

| Layer | A27 | gold+v6c | Notes |
|---|---|---|---|
| RELAXED_TOP_N | 8 | 8 | unchanged — 9 fails GSM8K under kimbochen |
| INT4 QuickReduce AR | ON | ON | unchanged |
| RCCL MSCCLPP knobs | ON | ON | unchanged |
| ATOM_MSCG_K | UNSET | UNSET | unchanged |
| `ATOM_USE_POST_ATTN_RMSNORM_MXFP4_FUSION` | 0 (or unset) | **1** | NEW — v6c env-gate |
| layernorm.py mxfp4 dispatch | absent | **wired** | NEW — calls `tensor_model_parallel_fused_allreduce_rmsnorm_mxfp4_quant` with size-guard |

**Net delta**: A27 1/4 (TPOT 6.171) → gold+v6c (TPOT 6.080 first run / 6.165 third run). Marginal -0.05 to -0.15 ms win, gate-adjacent on Intvty.

## Path to 4/4 (NOT yet achieved under kimbochen)

E2E gate (≤5000 ms) is the binding constraint. Need TPOT 6.08 → ~3.7 ms = -2.4 ms more. Realistic levers:

| Lever | ΔTPOT estimate | Status | Risk |
|---|---|---|---|
| **C1** MLA stage1+gather+cache fusion (HIP kernel) | -0.36 | 6-8 days, not started | low (numerics-preserving by construction) |
| **C2** MoE input prequant skip (v6e tensor-buffer redesign of v6d) | -0.5 to -1.0 | 2-3 days, v6d failed on cudagraph | medium |
| **C3** Spec V2 propose-loop overlap (pre-allocated buffer fix) | -0.2 to -0.4 | 2-3 days | medium |
| **R1** Cross-layer expert weight prefetch (BUFFER_LOAD_*_LDS) | -0.34 to -0.57 | 1-2 weeks | low |
| **R2** Small-M MoE GEMM kernel (V_MFMA_F32_16x16x128_F8F6F4 + DS_READ_B64_TR_B4) | -0.30 to -0.60 | 3-4 weeks | medium |

C1+C2+C3 stack = -1.06 to -1.76 ms → median 4.4-5.0 ms = **4/4 marginal**. C1+C2+C3+R1 stack = -1.4 to -2.3 ms → median 3.8-4.7 ms = **4/4 comfortable**. None of these is shipped yet.

---

# 🚨 OFFICIAL HARNESS CORRECTION (2026-04-27 ~06:15 IST) — kept for context

> Every prior "3/4 gates", "TPOT 4.84 ms", "thr/GPU 1650", "interactivity 207", and "E2E gap 240 ms" line in this document was based on a **non-official** bench formulation:
>   `--random-range-ratio 0.8`, no `--use-chat-template`, `--num-warmups 1`, num-prompts free-floating.
>
> The **actual scoring harness** is `kimbochen/dsr1_benchmark.cpp` from `github.com/danielhua23/amdgpu_bounty_optimization/blob/main/dsr1-fp4-atom-mtp-mi355x/dsr1_benchmark.cpp`. It:
>   1. Runs **GSM8K first** (`lm_eval --tasks gsm8k --num_fewshot 3 --model_args num_concurrent=65,max_retries=1,tokenized_requests=False`). Gate ≥ 0.93. **If GSM8K fails, perf phase is SKIPPED.**
>   2. Then runs perf bench with `--random-range-ratio 1.0`, `--use-chat-template`, `--num-warmups (2*CONC)`, `--num-prompts (CONC*10)`, `--max-concurrency CONC`.
>   3. Computes `tput_per_gpu = total_token_throughput / TP_size` (script hardcodes `/8` for TP=8; for our TP=4 we override to `/4`).
>
> **A26 baseline RE-MEASURED on 2026-04-27 06:13 under the OFFICIAL harness:**
>
...
> | Tput / GPU (tok/s/GPU, TP=4 → /4) | **1356.65** | 1650 | ≥1500 | ❌ FAIL (−10%) |
> | Interactivity (1000 / TPOT_med) | **156.97** | 206.61 | ≥165 | ❌ FAIL (−5%) |
>
> **REAL STATUS: 1/4 gates (only GSM8K passes).** All "3/4 gates" claims below this banner are wrong and only retained for archival traceability.
>
> **L2 v5 wired ON (Apr 27 ~05:43, single-shot):** GSM8K official = 0.9257 → FAIL. The numeric drift claim was based on single-shot only and is now suspect — needs N=3 re-run.
>
> **CORRECTION-OF-CORRECTION (Apr 27 ~13:25 IST):** GSM8K under official harness is NOISY (spread 0.0113 across same-stack reboots = larger than lm_eval stderr 0.0072). Single-shot is NOT a valid gate. **The new harness uses N=3 GSM8K runs + median ≥0.93.**
>
> **A26 (RELAXED_TOP_N=9) re-measured under N=3:** 0.9219 / 0.9348 / 0.9287 → median **0.9287 FAIL.** A26 does NOT pass official gates; the morning 0.9378 was an outlier on the high side.
>
> **A27 (RELAXED_TOP_N=8 = REAL BASELINE) under N=3 official harness, 13:25 IST:**
>
> | Metric | A27 | Gate | Status |
> |---|---|---|---|
> | GSM8K runs | 0.9356 / 0.9318 / 0.9348 | — | — |
...
>
> **A27 = 1/4 gates at CONC=4 and is now the canonical baseline.** Going forward all measurements MUST use `agents/harness/official_bench_n3.sh` (mirror inside container at `/tmp/official_bench_n3.sh`) which runs GSM8K ×3, takes median, and only proceeds to perf if median ≥0.93. No more single-shot, no more random-range=0.8, no more "3/4" shorthand.
>
> **A27 CONC=32 reference** (Apr 27 ~14:50 IST, same stack, `official_bench_n3_conc32.sh`):
>
...
> | TPOT_med | 17.80 ms | — | — |
>
> **CONC=32 = 2/4 gates** with sub-6% gaps on both failing metrics. Likely closes 4/4 *before* CONC=4 as levers stack.

---

# 📦 FINAL DELIVERY MANIFEST FOR AMD REVIEW (2026-04-26 EOD) — ⚠️ NUMBERS BELOW ARE PER WRONG INFORMAL HARNESS

## 🧬 Section 0 — STACK GENEALOGY (chronological build-up to current best)

⚠️ **The numbers in this section are from the WRONG informal harness.** Retained for chronology but should NOT be cited as performance proof. The official A26 baseline is in the correction block above.

This section traces every layer that was stacked from the **very-vanilla bounty baseline** to **today's "3/4-gates" result** (3/4 was an artifact of the informal harness; under the official harness A26 = 1/4). Every row cites the source it was extracted from. Cells with empty `—` are gaps in our records (we did not measure that metric at that stage).
...
| **MSCG P6 main+drafter graph wire (kv_indptr scratch + ATOM_MSCG_K=2 + ATOM_MSCG_P6_REPLAY=1)** | **DEFERRED (Apr 27 attempt 2)** | Apr 27 patched: `mscg_kv_indptr_scratch` buffer in model_runner.py, eagle.py i==0 copy-source-to-scratch, aiter_mla.py prepare_mtp_decode reads scratch directly. Scratch fix prevented the kv_indptr OOB GPU fault (smoke test passed cleanly with `ATOM_MSCG_P6_REPLAY=1` alone, `ATOM_MSCG_K=1`). But enabling `ATOM_MSCG_K=2` alongside (required for the P6 capture path to actually trigger) caused **TPOT 4.84 → 9.69 ms (+100% regression)**, thr/GPU 1650 → 848 (−49%), E2E 5240 → 10228 ms (+95%). Reverted. Backups `.pre_p6_scratch` preserved. Pre-snapshot `dsr1_apr27_pre_p6_revival` (sha 28a9a87efe88). DEFERRED for multi-day investigation: disentangle MSCG_K=2 engine-level multistep behavior from P6 cudagraph capture. Likely culprits: stale `max_seqlen_k` Python int frozen at capture, or engine-level multistep dispatching pure-decode-pairs that the MTP path can't actually pair correctly. |
| **L2 Phase 2 fusion (AR+RMSNorm+MXFP4-quant + dual BF16 unquant_out)** | **PARTIAL (Apr 27)** | KERNEL + DISPATCHER + PYTHON PLUMBING DONE — `ar_fusion_epilogue` MXFP4 branch now emits BF16 `unquant_out` as 4th output (single 16B vectorized global store, zero extra compute, reuses BF16 vector already in registers for FP4 conversion). 3 launchers (1stage/2stage/split) + dispatcher + macros all thread `unquant_out` through. Host `.cu` + `.h` + pybind binding + 4 Python plumbing files updated. Rebuilt `module_custom_all_reduce.so` (size 2364184). FP8 production path uses `nullptr` default — backward compatible. Patches in `patches/lever_progress/L2_*_apr27.diff`. WIRE NOT COMPLETE — `deepseek_v2.py` decoder branch + `DeepseekV2MoE._prequantized_input` routing scaffolded then reverted because `FusedMoE`/`routed_expert_forward` consumer of prequant payload wasn't written. Smoke test would boot but perf delta minimal (only saves post_attention_layernorm RMSNorm ~10µs/layer). Pre-snapshot `dsr1_apr27_pre_phase2_wire` (sha de5c5eb28f61). To complete: re-apply `.pre_L2_wire` backup + write `FusedMoE.forward_prequantized` + patch `Mxfp4MoEMethod.apply` with `x_prequantized` kwarg. |
| **L3 L4.5 Fuse_A_GEMM (relax use_triton_gemm guard)** | **DEFERRED (Apr 27)** | New env flag `ATOM_USE_FUSE_A_GEMM_AITER=1` in `envs.py` + relaxed guards at `deepseek_v2.py:1509-1511` + `:1527`. AITER `gemm_a16wfp4_preshuffle` kernel `_fuse_qkv_a_proj_reduce_rmsnorm_quant_fp4` reachable on FP4 path without `use_triton_gemm()`. Boot OK, smoke test sensible. 3-bench median: **TPOT 5.797 ms (+19.8% regression)**, thr/GPU 1321 (−19.9%), E2E 6224 ms (+18.8%). At M=4 (our CONC=4 shape), AITER's `gemm_a16wfp4_preshuffle` is slower than the unfused separate-GEMM path — different from agent's research expectation. Reverted. Backup `.pre_l4_5_relax` + `ATOM_USE_FUSE_A_GEMM_AITER` env flag retained for future. Pre-snapshot `dsr1_apr27_pre_l4_5` (sha 9f9da373a326). DEFERRED: needs (a) profile at M=4 to identify shuffle vs GEMM bottleneck, (b) verify shuffle/scale layout compat, (c) add tuned config entry in `aiter_configs/`. |
| **L4 MTP=4 V11 SAFE-MODE removal** | **NOT ATTEMPTED Apr 27** | Per agent research: V11 kernel header explicitly says "sq ∈ 1..8" — kernel IS designed-for, blocker is `SAFE-MODE early-continue` at v11.cuh:240-253 + `reduce_partial_map.q_start` allocates 1 slot/work_item instead of `sq` slots. Multi-day kernel work + correctness harness. Given Apr 27 pattern (3 levers attempted = 3 regressions/partial when wired naively), deferred to future session for proper kernel-level audit + standalone correctness harness. Patches not yet drafted. |
| **L5 DCP=4** | **NOT ATTEMPTED Apr 27** | Per agent research: DCP runtime IS already in ATOM/aiter (attention.py:679-1125, attention_mla.py:234-913, aiter/mla.py:27-411). Only ~135 LOC vLLM CLI-to-runtime port missing (config/parallel.py + config/model.py + parallel_state.py + utils.py + arg_utils.py). Multi-day work; lowest ROI per plan (−0.1 to −0.4 ms TPOT). Deferred. Patches not yet drafted. |
...
- [x] **Patches extracted with upstream SHA pin** → [`../patches/atom_apr26.diff`](../patches/atom_apr26.diff) (vs ATOM `f8453e3fc0f65191fb2034602dc9a2066a78020b`) and [`../patches/aiter_apr26.diff`](../patches/aiter_apr26.diff) (vs aiter `73ad0023e15e9735b3af95b3357b99cf7f801bf1`). Apply instructions in [`../patches/README.md`](../patches/README.md).
- [x] **Boot scripts captured verbatim** → [`../scripts/boot_apr26_tp4.sh`](../scripts/boot_apr26_tp4.sh), [`../scripts/boot_apr26_tp8.sh`](../scripts/boot_apr26_tp8.sh), [`../scripts/bench_apr26.sh`](../scripts/bench_apr26.sh). See [`../scripts/README.md`](../scripts/README.md).
- [x] **All 12 bench JSONs pushed as evidence** → [`../bench_results/apr26/`](../bench_results/apr26/). Index in [`../bench_results/apr26/README.md`](../bench_results/apr26/README.md).
- [x] **Phase 2 incremental patch scripts pushed** → [`../patches/scripts/`](../patches/scripts/) (7 idempotent Python scripts that apply the fusion patches to fresh upstream).
- [x] **Tag created**: [`dsr1-A26-evidence-apr26`](https://github.com/Danishlynx/AMD_DSR_CNCC4/releases/tag/dsr1-A26-evidence-apr26) marks the evidence-complete commit on GitHub.
- [ ] **GSM8K result JSON** — re-run scheduled (will land in `bench_results/apr26/gsm8k_a26.json`)
- [ ] **Push the Apr 26 patch as a single PR** onto `aiter` upstream (Phase 2 fusion 12 files + `block_convert.py` keystone)
- [ ] **Push the Apr 26 patch as a single PR** onto `ATOM` upstream (`rejection_sampler.py` + `envs.py`)
- [ ] **Push the lm_eval `outputs = None` fix** as a separate PR onto `lm-evaluation-harness`
- [ ] **Push the canonical snapshot** `rocm/atom-dev:dsr1_apr26_3of4_validated_warm_bench` (sha `8e844757ad6c`, 485 GB) to AMD's internal docker registry — only durability gap remaining, requires AMD coordination.
- [ ] **Note the dormant Phase 2 wiring** + `maybe_dual_stream_forward` signature change as a follow-up PR (multi-day, deferred).
