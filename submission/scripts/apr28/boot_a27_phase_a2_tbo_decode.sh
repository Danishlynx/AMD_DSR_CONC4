#!/bin/bash
# Phase A2 sub-test 1: --enable-tbo all (currently only prefill TBO is on)
LOGFILE=/tmp/a27_phase_a2_tbo_decode_$(date +%H%M%S).log
...
nohup python3 -m atom.entrypoints.openai_server \
  ...
  --method mtp --num-speculative-tokens 3 --enable-tbo all \
