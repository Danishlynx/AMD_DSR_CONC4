...
**Authoritative harness**: `kimbochen/dsr1_benchmark.cpp` -- official kimbochen 4-iter median(2,3,4) x N=3 boots, GSM8K N=3 median
...
| Throughput / GPU | **1449** | >= 1500 | FAIL (off 51) |
| Interactivity | **177.26** | >= 165 | PASS |
| Median E2E | 6210 ms | <= 5000 | FAIL (off 1210) |
| **Gates** | | | **2/4** |

This PR's lever -- **Phase 11 v3 (TRT-LLM thinking port)** -- delivers **-0.661 ms TPOT** measured under the official kimbochen N=3 harness on Apr 30. That -0.661 ms crosses Interactivity from 158.68 (FAIL) to 177.26 (PASS), the +1 gate this PR contributes. Env-gated NULL-OP at default: `ATOM_ENABLE_PER_PHASE_RELAXED_MTP=1` to activate.
...
|   |   |-- v1_initial.py / v2_triton_type_fix.py / v3_top8_outside_thinking_fix.py (KEEP)
...
| Lever | TPOT before | TPOT after | Delta | Measured |
|---|---:|---:|---:|---|
...
Other env flags / CLI configs in the production stack (e.g., `ATOM_ENABLE_RELAXED_MTP=1`, `--cudagraph-capture-sizes [1,2,4,8,16,32]`, RCCL_MSCCLPP knobs) were turned ON during the Apr 14--26 informal-bench era of the campaign; **their per-lever delta-TPOT numbers from that era are historical, not authoritative under the canonical harness** -- see [`PERFORMANCE_METRICS.md`](PERFORMANCE_METRICS.md) for the regime-separated breakdown.
